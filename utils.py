...
def get_scores(filepath,learn, ret=False):
    ...
    dict_data = {}
    dict_data['Accuracy']= [(tp+tn)/len(y)]
    dict_data['False Positives'] = [fp/len(y)]
    dict_data['False Negatives'] = [fn/len(y)]
    dict_data['Recall'] = [tp / (tp + fn)]
    dict_data['Precision'] = [tp / (tp + fp)]
    dict_data['Specificity'] = [tn / (tn + fp)]
    dict_data['MCC']= [cc]
    #print(dict_data)
...
class SequenceShuffler():
    ...
    def rand_sequence(self, length):
        return ''.join(np.random.choice('CGTA') for _ in range(length))
